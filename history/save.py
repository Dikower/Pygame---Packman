...
class Game:
    ...
    def get_coors_on_board_with_bias(self, sprite):
        ...
        # if sprite.direction == 3 or sprite.direction == 4:
        #     return (sprite.rect.x + self.CELL_SIZE - camera_bias_x)//self.CELL_SIZE - 1, \
        #         (sprite.rect.y + self.CELL_SIZE - camera_bias_y)//self.CELL_SIZE - 1  # = (x, y)
    ...
    def centry_sprite(self, sprite):
        ...
        # difference_x = cell_rect.x - sprite.rect.x  # +
        # difference_y = cell_rect.y - sprite.rect.y
        # iterations_for_x = abs(difference_x // sprite.speed)
        # iterations_for_y = abs(difference_y // sprite.speed)
        # if iterations_for_x != 0:
        #     print("sprite:", sprite.rect.x, sprite.rect.y)
        #     print("cell_pix:", cell_rect.x, cell_rect.y)
        #     print("cell_on_board:", cell_x_on_board, cell_y_on_board)
        #     print("diff:", difference_x, difference_y)
        #     print("speed:", sprite.speed)
        #     print("iter:", iterations_for_x)
        #     print("-" * 1000)
        # difference_speed_for_animation = 10
        # for iteration in range(iterations_for_x):
        #     if difference_x < 0:
        #         sprite.rect.x -= sprite.speed - difference_speed_for_animation
        #     elif difference_x > 0:
        #         sprite.rect.x += sprite.speed + difference_speed_for_animation
        #     self.render()
        #
        # for iteration in range(iterations_for_y):
        #     if difference_y < 0:
        #         sprite.rect.y -= sprite.speed - difference_speed_for_animation
        #     elif difference_y > 0:
        #         sprite.rect.y -= sprite.speed + difference_speed_for_animation
        #     self.render()
    ...
    def update_hero(self, event):
        ...
        if check_new_direction and (self.hero.direction != self.hero.new_direction):
            ...
            # print(self.hero.direction, check_direction)
            # print("-"*1000)
            # print(self.board[self.hero.coors[1]-1])
            # print(self.board[self.hero.coors[1]])
            # print(self.board[self.hero.coors[1]+1])
            # print("-" * 1000)
            # cells_for_random_choice = []
            # possible_cells = [(0, 1), (1, 0), (-1, 0), (0, -1)]
            # for possible in possible_cells:
            #     new_x = self.hero.coors[0] + possible[0]
            #     new_y = self.hero.coors[1] + possible[1]
            #     if -1 < new_x < self.height and -1 < new_y < self.width:
            #         if self.board[new_y][new_x] != "#":
            #             cells_for_random_choice.append(possible)
            # if len(cells_for_random_choice) != 0:
            #     self.hero.random_moving(cells_for_random_choice)
            #     changed_direction = True
            # else:
            #     return 1
